...
class CareerIntelligenceEngine:
    ...
    def __init__(self):
        ...
        # Canadian job market data (realistic synthetic data for modeling)
    ...
    def _initialize_market_data(self) -> pd.DataFrame:
        """Initialize realistic Canadian job market dataset"""
        np.random.seed(42)
        n_samples = 2000
        ...
        for i in range(n_samples):
            ...
            # Realistic salary modeling based on Canadian market
            base_salary = 55000
            city_multiplier = {'Toronto': 1.2, 'Vancouver': 1.15, 'Montreal': 1.0, 
                             'Ottawa': 1.1, 'Calgary': 1.08, 'Edmonton': 1.05}[city]
            ...
            salary = base_salary * city_multiplier * exp_multiplier * industry_multiplier * education_multiplier
            salary += np.random.normal(0, 8000)  # Add realistic variance
            salary = max(45000, min(300000, salary))  # Realistic bounds
    ...
    def _generate_market_analysis(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'city_analysis': {
                'avg_salary': round(city_data['salary_cad'].mean(), 0),
                'job_opportunities': 'High' if len(city_data) > 300 else 'Medium',
                'remote_work_rate': round(city_data['remote_work_available'].mean() * 100, 1),
                'competition_level': 'High' if city == 'Toronto' else 'Medium'
            },
            'industry_analysis': {
                'avg_salary': round(industry_data['salary_cad'].mean(), 0),
                'growth_trend': 'Growing' if industry in ['Tech', 'Finance'] else 'Stable',
                'skill_demand': ['Python', 'SQL', 'Machine Learning', 'Communication'],
                'hiring_rate': round(industry_data['hired'].mean() * 100, 1)
            },
            'recommendations': [
                'Focus on high-demand skills',
                'Build a strong portfolio',
                'Network in your target city',
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; margin-bottom: 30px;">
                    ...
                    <div style="background: rgba(255,255,255,0.95); border-radius: 15px; padding: 25px; color: #333;">
                        <h3 style="margin: 0 0 20px 0; color: #667eea; display: flex; align-items: center; gap: 10px;">
                            <span style="font-size: 1.3em;">🏙️</span> {profile.get('city', 'Toronto')} Market
                        </h3>
                        <div style="display: grid; gap: 12px;">
                            <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #eee;">
                                <span>Average Salary:</span>
                                <strong>CAD ${insights['market_analysis']['city_analysis']['avg_salary']:,.0f}</strong>
                            </div>
                            <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #eee;">
